...
/* If user cancels edit of user info, redirect user to their blogs page */
if( isset( $_POST["cancel"] ) ) {
	header("Location: blogs.php");
}

/* If user clicks to delete their account */
...
/* Select all data for current user in DB, to display to the user */
$query 	= "SELECT *
		  FROM users
		  WHERE id = '$user_id'";
$result = mysqli_query( $conn, $query );
...
/* If user has not set a user avatar or for some reason $avatar is not set in the DB for the user, set user image to default image */
if( !$avatar ) {
...
/* If user Saves edit of user info, Update user info in DB */
if( isset( $_POST['save'] ) ) {
//	var_dump($_POST);
//	var_dump($_FILES);
...
//				$image_ext = pathinfo( $user_image, PATHINFO_EXTENSION );
				$avatar = $user_image;
//				clearstatcache(TRUE, $_SERVER['SCRIPT_NAME']);
